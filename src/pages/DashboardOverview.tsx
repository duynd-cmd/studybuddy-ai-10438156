...
export default function DashboardOverview() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* ROW 1 — Hero progress + Streak (equal heights via grid) */}
      <StaggerReveal className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        <TiltCard className="lg:col-span-2" maxTilt={3}>
          <GlassCard variant="elevated" className="h-full p-6 flex flex-col gap-5 border-glow overflow-hidden">
            <CardTitleRow icon={TrendingUp} label="Tiến độ tổng quan" />
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <LiquidProgress value={accuracy} size={150} label="Tiến độ" />
            ...
            <div className="mt-auto">
              <MagneticButton size="sm" onClick={() => navigate("/dashboard/ke-hoach")} className={primaryBtn}>
        ...
        <TiltCard maxTilt={4}>
          <GlassCard variant="interactive" className="h-full p-6 flex flex-col gap-4 relative overflow-hidden">
            ...
            <div className="mt-auto h-1.5 rounded-full bg-muted/50 overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
      ...
      {/* ROW 2 — Compact stats (2 cards) */}
      <StaggerReveal className="grid grid-cols-2 gap-6 items-stretch">
        <TiltCard maxTilt={4}>
          <GlassCard variant="interactive" className="h-full p-6 flex flex-col gap-3">
            <CardTitleRow icon={Clock} label="Phút tập trung" />
        ...
        <TiltCard maxTilt={4}>
          <GlassCard variant="interactive" className="h-full p-6 flex flex-col gap-3">
            <CardTitleRow icon={HelpCircle} label="Câu hỏi đã hỏi" />
            <p className="text-3xl font-heading font-bold text-foreground leading-none">{messageCount || 0}</p>
            <p className="text-xs text-muted-foreground">cùng Scriba AI</p>
          </GlassCard>
        </TiltCard>
      </StaggerReveal>

      {/* ROW 3 — Plan + Notes (equal heights) */}
      <StaggerReveal className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
        <TiltCard maxTilt={3}>
          <GlassCard variant="interactive" className="h-full p-6 flex flex-col gap-4">
              ...
              {recentNotes && recentNotes.length > 0 ? (
                recentNotes.map((n: any) => (
                  <button
                    key={n.id}
                    onClick={() => navigate("/dashboard/ghi-chu")}
                    className="w-full text-left p-3 rounded-lg bg-secondary/40 hover:bg-secondary/70 transition-colors"
                  >
      ...
      {/* ROW 4 — Quick actions (3 equal cards) */}
      <StaggerReveal className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
        {[
          { icon: Clock, label: "Pomodoro", title: "Phiên 25 phút", sub: "Tập trung sâu", cta: "Bắt đầu", to: "/dashboard/pomodoro" },
          { icon: Brain, label: "Scriba AI", title: "Hỏi trợ lý AI", sub: "Giải đáp tức thì", cta: "Mở chat", to: "/dashboard/scriba" },
          { icon: TrendingUp, label: "Kế hoạch mới", title: "AI lập lộ trình", sub: "Cá nhân hoá theo bạn", cta: "Tạo ngay", to: "/dashboard/ke-hoach" },
        ].map((a) => (
          <TiltCard key={a.label} maxTilt={4}>
            <GlassCard variant="interactive" className="h-full p-6 flex flex-col gap-3">
              <CardTitleRow icon={a.icon} label={a.label} />
              <div className="flex flex-col gap-1">
                <p className="text-base font-heading font-bold text-foreground">{a.title}</p>
                <p className="text-xs text-muted-foreground">{a.sub}</p>
              </div>
              <div className="mt-auto">
                <MagneticButton size="sm" onClick={() => navigate(a.to)} className={`${primaryBtn} w-full`}>
                  {a.cta}
                </MagneticButton>
              </div>
            </GlassCard>
